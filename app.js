...
function navigateToMacro(macroNumber) {
  ...
  setTimeout(() => {
    ...
    if (macroNumber >= 13 && macroNumber <= 17) {
      subtabId = 'pue-macros';
...
const SearchManager = {
  ...
  buildAllTabsTextIndex() {
    ...
    const getHtml = (pageId) => {
      ...
      switch (pageId) {
        ...
        case "nevolat-exclusion":
          return ContentManager.getNevolatExclusionContent();
        ...
        case "bmi-calculator":
          return ContentManager.getBMICalculatorContent();
        default:
          return "";
      }
    };
  ...
  highlightTermsInScope(scopeEl, terms, highlightClass) {
    ...
    const wrapMatchesInTextNode = (textNode, term) => {
      ...
      // Find the containing tabs for this text node
      const parentEl = textNode.parentElement;
      const protocolTab = parentEl ? parentEl.closest('.protocol-tab-content[data-tab-content]') : null;
      const scrTab = parentEl ? parentEl.closest('.scr-tab-content[data-condition-content]') : null;
      
      const protocolTabId = protocolTab ? protocolTab.dataset.tabContent : null;
      const scrTabId = scrTab ? scrTab.dataset.conditionContent : null;
      ...
      matches.forEach((m) => {
        ...
        // Store tab ancestry metadata on the mark for later use
        if (protocolTabId) mark.dataset.protocolTab = protocolTabId;
        if (scrTabId) mark.dataset.scrTab = scrTabId;
  ...
  updateOccurrenceUI(mode) {
    ...
    // For global mode, check if there are matches on other pages even if current page has 0
    const hasOtherPageMatches = mode === "global" && 
      this.latestFindSummary && 
      this.latestFindSummary.allTabsTotal > 0;

    if (total === 0 && !hasOtherPageMatches) {
      controlsEl.style.display = "none";
      countEl.textContent = "0/0";
      return;
    }

    // Show controls if there are matches (on this page or other pages)
    controlsEl.style.display = "flex";
    const current = state.index >= 0 ? state.index + 1 : 0;

    let displayText = "";
    if (mode === "global" && this.latestFindSummary) {
      const { allTabsTotal, perPage } = this.latestFindSummary;
      const pagesWithMatches = (perPage || []).filter(p => p.count > 0).length;
      
      if (total === 0 && allTabsTotal > 0) {
        // No matches on current page but matches exist elsewhere
        displayText = `0 here • ${allTabsTotal} in ${pagesWithMatches} page${pagesWithMatches > 1 ? 's' : ''}`;
      } else if (allTabsTotal > total) {
        // Matches on current page plus more on other pages
        displayText = `${current}/${total} • ${allTabsTotal} total`;
      } else {
  ...
  setActiveOccurrence(mode) {
    ...
    if (mark) {
      // Open containing tabs before scrolling to the mark
      this.openTabsForMark(mark);
      ...
      // Small delay to allow tabs to render before scrolling
      setTimeout(() => {
  ...
  // Open the tabs containing a marked search result
  openTabsForMark(mark) {
    if (!mark) return;
    
    const protocolTabId = mark.dataset.protocolTab;
    const scrTabId = mark.dataset.scrTab;
    
    // Open main protocol tab if needed
    if (protocolTabId) {
      const tabContent = document.querySelector(`.protocol-tab-content[data-tab-content="${protocolTabId}"]`);
      if (tabContent && !tabContent.classList.contains('active')) {
        const tabsContainer = tabContent.closest('.scr-protocol, .protocol-content');
        if (tabsContainer) {
          // Deactivate all tabs
          tabsContainer.querySelectorAll('.protocol-tab').forEach(t => t.classList.remove('active'));
          tabsContainer.querySelectorAll('.protocol-tab-content').forEach(c => c.classList.remove('active'));
          
          // Activate the correct tab
          const tabButton = tabsContainer.querySelector(`.protocol-tab[data-tab="${protocolTabId}"]`);
    ...
    // Open nested SCR condition tab if needed
    if (scrTabId) {
      const conditionContent = document.querySelector(`.scr-tab-content[data-condition-content="${scrTabId}"]`);
      if (conditionContent && !conditionContent.classList.contains('active')) {
        const tabbedContainer = conditionContent.closest('.scr-tabbed-conditions');
        if (tabbedContainer) {
          // Deactivate all condition tabs
          tabbedContainer.querySelectorAll('.scr-tab').forEach(t => t.classList.remove('active'));
          tabbedContainer.querySelectorAll('.scr-tab-content').forEach(c => c.classList.remove('active'));
          
          // Activate the correct condition tab
          const conditionButton = tabbedContainer.querySelector(`.scr-tab[data-condition="${scrTabId}"]`);
          if (conditionButton) conditionButton.classList.add('active');
          conditionContent.classList.add('active');
        }
      }
    }
  },
  ...
  gotoOccurrence(direction, mode = "global") {
    ...
    // If no marks on current page but there are matches on other pages (global mode only)
    if (!state.marks.length && mode === "global" && this.latestFindSummary) {
      const { perPage, rawQuery } = this.latestFindSummary;
      if (perPage && perPage.length > 0) {
        // Navigate to first/last page with matches depending on direction
        const targetPage = direction > 0 ? perPage[0] : perPage[perPage.length - 1];
        if (targetPage && targetPage.pageId !== AppState.currentPage) {
          this.navigateToPageWithSearch(targetPage.pageId, rawQuery, direction > 0 ? 'first' : 'last');
        }
      }
      return;
    }
    ...
    // Check if we're at the boundary and should navigate to another page (global mode only)
    if (mode === "global" && this.latestFindSummary) {
      const { perPage, rawQuery } = this.latestFindSummary;
      const currentPageId = AppState.currentPage;
      
      // Find current page position in results
      const pagesWithMatches = perPage.filter(p => p.count > 0);
      const currentPageIndex = pagesWithMatches.findIndex(p => p.pageId === currentPageId);
      
      // Moving forward from last match
      if (direction > 0 && state.index === total - 1 && pagesWithMatches.length > 1) {
        const nextPageIndex = (currentPageIndex + 1) % pagesWithMatches.length;
        const nextPage = pagesWithMatches[nextPageIndex];
        if (nextPage && nextPage.pageId !== currentPageId) {
          this.navigateToPageWithSearch(nextPage.pageId, rawQuery, 'first');
          return;
        }
      }
      
      // Moving backward from first match
      if (direction < 0 && state.index === 0 && pagesWithMatches.length > 1) {
        const prevPageIndex = (currentPageIndex - 1 + pagesWithMatches.length) % pagesWithMatches.length;
        const prevPage = pagesWithMatches[prevPageIndex];
        if (prevPage && prevPage.pageId !== currentPageId) {
          this.navigateToPageWithSearch(prevPage.pageId, rawQuery, 'last');
          return;
        }
      }
    }
  ...
  // Navigate to a page and re-run search, positioning at first or last occurrence
  navigateToPageWithSearch(pageId, rawQuery, position = 'first') {
    // Store search info to restore after navigation
    this.pendingSearchRestore = { rawQuery, position };
    
    // Navigate to the page
    NavigationManager.goToPage(pageId);
    
    // Re-run search on the new page after a short delay for DOM to update
    setTimeout(() => {
      const searchInput = document.getElementById("globalSearch");
      if (searchInput) {
      ...
      // Position at first or last occurrence
      if (this.globalFindState.marks.length > 0) {
        if (position === 'last') {
          this.globalFindState.index = this.globalFindState.marks.length - 1;
        } else {
          this.globalFindState.index = 0;
        }
        this.setActiveOccurrence("global");
      }
      
      this.pendingSearchRestore = null;
    }, 200);
  },
  ...
  buildSearchIndex() {
    this.searchData = [
      ...
      {
        title: "Retinopathy",
        context: "Active retinopathy - REJECT immediately",
        page: "contraindications",
        sectionId: "cardiac",
        protocolTab: "absolute",
        category: "Absolute Contraindication",
        keywords: ["retinopathy", "diabetic retinopathy", "active retinopathy", "eye", "vision"],
      },
      {
        title: "MEN2",
      ...
      },
      {
        ...
        protocolTab: "patient-assessment",
      ...
      .forEach((item, index) => {
        item.addEventListener("click", () => {
          ...
          this.pendingNavigation = {
            ...
            protocolTab: result.protocolTab || null,
            searchTitle: result.title || null,
  ...
  onPageRendered(pageId) {
    ...
    // If navigation was triggered by search, open tabs and scroll to match
    if (this.pendingNavigation && this.pendingNavigation.pageId === pageId) {
      const { sectionId, protocolTab, searchTitle, rawQuery } = this.pendingNavigation;
      
      // First, open the protocol tab if specified
      setTimeout(() => {
        // Open protocol tab directly if specified
        if (protocolTab) {
          this.openProtocolTabById(protocolTab);
        } else if (sectionId) {
          // Fall back to trying to find by sectionId
          this.openTabsForSection(sectionId);
        }

        // After tab is opened, wait for render then find and scroll to the matching content
        setTimeout(() => {
          // Try to find the highlighted mark for the search term
          const marks = document.querySelectorAll('mark.find-highlight');
          let targetMark = null;
          
          // Find a mark that contains content related to the search
          if (marks.length > 0) {
            // If we have a searchTitle, try to find a mark near that title text
            if (searchTitle) {
              for (const mark of marks) {
                const container = mark.closest('.condition-group, .info-card, .protocol-card, .rejection-group, p, li, td');
                if (container && container.textContent.toLowerCase().includes(searchTitle.toLowerCase())) {
                  targetMark = mark;
                  break;
                }
              }
            ...
            // If not found by title, use the first visible mark
            if (!targetMark) {
              for (const mark of marks) {
                const rect = mark.getBoundingClientRect();
                // Check if mark would be visible (not in hidden tab)
                const tabContent = mark.closest('.protocol-tab-content, .scr-tab-content');
                if (!tabContent || tabContent.classList.contains('active')) {
                  targetMark = mark;
                  break;
                }
            ...
            // Still not found? Just use first mark
            if (!targetMark) {
              targetMark = marks[0];
            }
          }
          ...
          if (targetMark) {
            // Open tabs for this mark if needed
            this.openTabsForMark(targetMark);
            
            setTimeout(() => {
              targetMark.scrollIntoView({ behavior: "smooth", block: "center" });
              targetMark.style.animation = "highlight-pulse 2s ease-in-out";
              setTimeout(() => {
                targetMark.style.animation = "";
              }, 2000);
            }, 100);
          } else if (sectionId) {
            // Fallback: try to find section by ID
  ...
  // Open a protocol tab by its ID directly
  openProtocolTabById(tabId) {
    const tabContent = document.querySelector(`.protocol-tab-content[data-tab-content="${tabId}"]`);
    if (!tabContent) return;
    
    const container = tabContent.closest('.protocol-content, .scr-protocol') || tabContent.parentElement;
    if (!container) return;
    
    // Deactivate all tabs and contents in this container
    container.querySelectorAll('.protocol-tab').forEach(t => t.classList.remove('active'));
    container.querySelectorAll('.protocol-tab-content').forEach(c => c.classList.remove('active'));
    
    // Activate the target tab
    const tabButton = container.querySelector(`.protocol-tab[data-tab="${tabId}"]`);
    if (tabButton) tabButton.classList.add('active');
    tabContent.classList.add('active');
  },
...
const NavigationManager = {
  ...
  init() {
    ...
    document.querySelectorAll(".subnav-item[data-page]").forEach((item) => {
      item.addEventListener("click", () => {
        const pageId = item.dataset.page;
        const tabId = item.dataset.tab;
        this.goToPageWithTab(pageId, tabId);
      });
    });
  ...
  updateHeader(pageId) {
    const titles = {
      ...
      "nevolat-exclusion": ["Nevolat Specific Exclusion", "Critical exclusion criteria for Nevolat prescribing"],
      ...
      "bmi-calculator": ["BMI Calculator", "Medical-grade BMI calculation with ethnicity-adjusted thresholds"],
    };
...
const ContentManager = {
    ...
    setTimeout(() => {
      ...
      switch (pageId) {
        ...
        case "contraindications":
          ...
          break;
        case "nevolat-exclusion":
          content = this.getNevolatExclusionContent();
          break;
        case "titration-guide":
        ...
        case "macros":
          ...
          break;
        case "bmi-calculator":
          content = this.getBMICalculatorContent();
          break;
        default:
        ...
        },
        {
          ...
          checks: [
            ...
            },
            {
              ...
              hint: "Use <a href='#macro-14' onclick='navigateToMacro(14); return false;' style='color: var(--accent);'>Macro 14: Previous Use Evidence</a>. Add 'Pending Customer Response'. Without valid PUE → starter dose only.",
              warning: true,
            },
          ],
        },
    ...
    return `
      <div class="scr-protocol">
        <div class="protocol-tabs">
          ...
          <button class="protocol-tab" data-tab="nevolat-exclusion">🚨 Nevolat Exclusion</button>
        ...
        <div class="protocol-tab-content" data-tab-content="nevolat-exclusion">
          <div class="protocol-card" style="border: 3px solid var(--danger); background: rgba(239, 68, 68, 0.05);">
            <div class="protocol-title">
              <div class="icon red">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              CRITICAL: Nevolat Thyroid & Liver Disease Exclusion
            </div>
            <div class="protocol-text" style="font-size: 16px; font-weight: 600; color: var(--danger); margin-bottom: 16px;">
              Patients with ANY thyroid disease OR liver disease/impairment CANNOT receive Nevolat.
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 20px;">
              <div style="padding: 16px; background: rgba(16, 185, 129, 0.1); border-radius: 8px; border: 2px solid rgba(16, 185, 129, 0.3);">
                <div style="font-weight: 700; color: var(--success); margin-bottom: 10px; font-size: 15px;">✅ These patients CAN receive:</div>
                <ul class="protocol-list">
                  <li><strong>Mounjaro</strong> (tirzepatide)</li>
                  <li><strong>Wegovy</strong> (semaglutide)</li>
                </ul>
              </div>
              <div style="padding: 16px; background: rgba(239, 68, 68, 0.1); border-radius: 8px; border: 2px solid rgba(239, 68, 68, 0.4);">
                <div style="font-weight: 700; color: var(--danger); margin-bottom: 10px; font-size: 15px;">❌ These patients CANNOT receive:</div>
                <ul class="protocol-list">
                  <li><strong>Nevolat</strong> (liraglutide)</li>
                </ul>
              </div>
            </div>

            <div style="padding: 16px; background: var(--bg-elevated); border-radius: 8px; margin-bottom: 16px;">
              <div style="font-weight: 700; margin-bottom: 12px; font-size: 15px;">Action if patient ordered Nevolat:</div>
              <ol class="protocol-ol">
                <li><strong>REJECT</strong> the Nevolat order immediately</li>
            ...
            <div style="padding: 12px 16px; background: rgba(245, 158, 11, 0.1); border-radius: 6px; border-left: 4px solid var(--warning);">
              <strong style="color: var(--warning);">⚠️ Note:</strong> CTP (Choose Treatment Page) automatically hides Nevolat for patients who answered "Yes" to thyroid/liver question. If patient somehow ordered Nevolat anyway, this is an error that must be corrected.
            </div>
          </div>
        </div>
    ...
    return `
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          ...
          <div class="info-card blue" style="margin-top: 12px;">
            <div class="info-card-title">📧 Related Macro</div>
            <div class="info-card-text">
              <a href="#macro-21" onclick="navigateToMacro(21); return false;" class="tag blue">Macro 21: Failed ID</a> — Automated email sent when Failed ID tag is applied.
            </div>
          </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          ...
          <div class="info-card blue" style="margin-top: 12px;">
            <div class="info-card-title">📧 Related Macro</div>
            <div class="info-card-text">
              <a href="#macro-22" onclick="navigateToMacro(22); return false;" class="tag blue">Macro 22: Weight Verification Failed</a> — Automated email sent when tag is applied.
            </div>
          </div>
        </div>
        <div class="info-card red" style="margin-top: 12px;">
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          ...
          <div class="info-card blue" style="margin-top: 12px;">
            <div class="info-card-title">📧 Related Macros</div>
            <div class="info-card-text">
              <strong>Incomplete GP:</strong> Automated email sent when Incomplete GP details tag is applied.<br>
              <strong>No UK GP:</strong> <a href="#macro-27" onclick="navigateToMacro(27); return false;" class="tag blue">Macro 27: Standard Rejection</a> — For rejecting orders from patients without UK GP.
            </div>
          </div>
        </div>
        <div class="info-card red" style="margin-top: 12px;">
    ...
    return `
      ...
      <div class="protocol-card">
        ...
        <div class="info-card blue" style="margin-top: 16px;">
          <div class="info-card-title">📧 Request PUE or Previous Weight Photo</div>
          <div class="info-card-text">
            <div style="display: flex; flex-direction: column; gap: 8px;">
              <div><a href="#macro-13" onclick="navigateToMacro(13); return false;" class="tag blue">Macro 13: PUE &amp; BMI Photo</a> — Request proof of previous use from transfer patient</div>
              <div><a href="#macro-15" onclick="navigateToMacro(15); return false;" class="tag blue">Macro 15: Previous BMI Photo</a> — Request previous weight photo (for below-licence BMI)</div>
              <div><a href="#macro-32" onclick="navigateToMacro(32); return false;" class="tag purple">Macro 32: Combined Request</a> — Request BOTH PUE AND previous weight photo together</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          ...
          <div class="protocol-text">
            If either PUE or previous BMI photo fails requirements, patient is not eligible. Use <a href="#macro-15" class="tag blue" onclick="navigateToMacro(15); return false;">Macro 15: Previous BMI Photo</a>.
          </div>
        </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          ...
          <div class="info-card blue" style="margin-top: 12px;">
            <div class="info-card-title">📧 If PUE is Missing or Incomplete</div>
            <div class="info-card-text">
              Use <a href="#macro-14" onclick="navigateToMacro(14); return false;" class="tag blue">Macro 14: Previous Use Evidence</a> — Request the missing PUE details from the patient. Add <span class="tag orange">Pending Customer Response</span> tag.
            </div>
          </div>
        </div>
        <div class="divider"></div>
        ...
        <div class="info-card green" style="margin-top: 16px;">
          <div class="info-card-title">📧 Related PUE Macros Reference</div>
          <div class="info-card-text">
            <div style="display: flex; flex-direction: column; gap: 8px;">
              <div><a href="#macro-13" onclick="navigateToMacro(13); return false;" class="tag blue">Macro 13: PUE &amp; BMI Photo</a> — Request PUE from transfer patient</div>
              <div><a href="#macro-14" onclick="navigateToMacro(14); return false;" class="tag blue">Macro 14: Previous Use Evidence</a> — Request missing/incomplete PUE details</div>
              <div><a href="#macro-15" onclick="navigateToMacro(15); return false;" class="tag blue">Macro 15: Previous BMI Photo</a> — Request evidence of starting BMI (for below-licence patients)</div>
              <div><a href="#macro-16" onclick="navigateToMacro(16); return false;" class="tag blue">Macro 16: PUE 2 Weeks Old</a> — PUE shows order within last 2 weeks (approve but send)</div>
            </div>
          </div>
        </div>
      </div>
  ...
  getReviewsProtocolCards() {
    return `
      ...
      <div class="protocol-tabs" style="flex-wrap: wrap;">
        ...
        <button class="protocol-tab" data-tab="triage">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
            <line x1="12" y1="9" x2="12" y2="13"/>
            <line x1="12" y1="17" x2="12.01" y2="17"/>
        ...
        <button class="protocol-tab" data-tab="checks">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M9 11l3 3L22 4"/>
            <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/>
          </svg>
        ...
        <button class="protocol-tab" data-tab="weight-actions">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/>
            <path d="M14 2v6h6"/>
        ...
        <button class="protocol-tab" data-tab="exemptions">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="12" cy="12" r="10"/>
        ...
        <button class="protocol-tab" data-tab="closing">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M22 11.08V12a10 10 0 11-5.93-9.14"/>
            <polyline points="22 4 12 14.01 9 11.01"/>
          </svg>
        ...
        <button class="protocol-tab" data-tab="cheatsheet">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/>
          </svg>
          Quick Reference
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-text">
          The periodic review is a <strong>structured checkpoint</strong> to decide whether a patient can <strong>continue GLP-1</strong> under UK product licence expectations. It ensures ongoing treatment is clinically appropriate, safe, and beneficial.
        </div>
        <div class="info-card blue" style="margin-top: 12px;">
          <div class="info-card-title">The Clock Starts From</div>
          <div class="info-card-text">
            The customer's <strong>first GLP-1 order — even if it was with another provider</strong>. The schedule <strong>resets when a patient switches GLP-1 drug</strong>.
          </div>
        </div>
      </div>
        ...
        <div class="table-wrapper">
          <table class="dose-table">
            <tr>
              <th>Medication</th>
              <th>Review Due</th>
              <th>Window to Complete</th>
              <th>If Not Completed</th>
            </tr>
            <tr>
              <td><strong>Mounjaro / Wegovy</strong></td>
              <td>After 6 months use</td>
              <td>Month 6 to Month 10</td>
              <td>Orders held from Month 10</td>
            </tr>
            <tr>
              <td><strong>Nevolat (liraglutide)</strong></td>
              <td>After 4 months use</td>
              <td>Month 4 to Month 8</td>
              <td>Orders held from Month 8</td>
            </tr>
          </table>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Who Gets Forced Into Periodic Review?
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">Required (NOT achieved ≥5% weight loss)</div>
          <ul class="protocol-list">
            <li><strong>Mounjaro/Wegovy:</strong> Must complete between Month 6 and Month 10</li>
            <li><strong>Nevolat:</strong> Must complete between Month 4 and Month 8</li>
          </ul>
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">Optional (HAVE achieved ≥5% weight loss)</div>
          <ul class="protocol-list">
            <li><strong>Mounjaro/Wegovy:</strong> Optional at 6-monthly intervals</li>
            <li><strong>Nevolat:</strong> Optional at 4-monthly intervals</li>
          </ul>
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">Transfer Patients</div>
          <ul class="protocol-list">
            <li><strong>Mounjaro/Wegovy:</strong> If 6+ months elsewhere → review requested on <strong>first order with you</strong></li>
            <li><strong>Nevolat:</strong> If 4+ months elsewhere → review requested on <strong>first order with you</strong></li>
          </ul>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          What the Patient Fills In (Questionnaire)
        </div>
        <div class="protocol-text">
          The periodic review questionnaire asks:
        </div>
        <ul class="protocol-list">
          <li>Are you losing weight at a rate you're happy with?</li>
          <li>Side effects in last 2 weeks (some flagged as <strong>urgent</strong>)</li>
          <li>New medications / new diagnoses since starting</li>
          <li>Health benefits noticed (waist, mood, mobility, etc.)</li>
          <li>Weekly injection schedule adherence</li>
          <li>Diet changes</li>
          <li>Activity changes</li>
          <li>Anything else that may impact weight loss</li>
        </ul>
        <div class="info-card red" style="margin-top: 12px;">
          <div class="info-card-title">Important: No Photo Required</div>
          <div class="info-card-text">
            Do <strong>NOT</strong> request a photo for periodic review. Photos are required at initial order only, <strong>not</strong> for continuing at periodic review.
          </div>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          What Happens If Patient Ignores Review
        </div>
        <ul class="protocol-list">
          <li>Patient is <strong>auto-notified via CRM</strong> to complete in their dashboard</li>
          <li>If no response by deadline → <strong>subsequent orders placed on hold</strong></li>
          <li><span class="tag red">incomplete periodic review</span> tag added automatically</li>
          <li>Customer Support follows them up</li>
        </ul>
        <div class="info-card blue" style="margin-top: 12px;">
          <div class="info-card-title">Prescriber Experience Requirement</div>
          <div class="info-card-text">
            Prescribers won't be expected to prescribe periodic review orders unless they have been prescribing GLP-1 medications for MedExpress for <strong>more than 2 months</strong>.
          </div>
        </div>
      </div>
      ...
      <!-- Tab 2: Step A - Triage -->
      <div class="protocol-tab-content" data-tab-content="triage">
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-text">
          If the patient selected an <strong>urgent side effect</strong> (flagged options), the periodic review appears in the <strong>Urgent queue</strong>.
        </div>
        <div class="info-card red" style="margin-top: 12px;">
          <div class="info-card-title">Urgent Side Effects — Action Required</div>
          <div class="info-card-text">
            Must be reviewed by a <strong>Senior Prescriber within 8 working hours</strong>. The customer may need urgent medical input.
          </div>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          If Urgent Side Effect Flagged
        </div>
        <ul class="protocol-list">
          <li><strong>1.</strong> Telephone contact <strong>within 8 working hours</strong></li>
          <li><strong>2.</strong> If you can't reach → Add tag <span class="tag yellow">Pending Customer Response</span> and email them</li>
          <li><strong>3.</strong> Submit a <strong>Patient Safety Incident (PSI)</strong>:
            <ul style="margin-top: 8px; margin-left: 20px;">
              <li>Category: <strong>Clinically Significant Side Effects</strong></li>
              <li>Select correct option for "Did these side effects require medical attention?"</li>
            </ul>
          </li>
          <li><strong>4.</strong> Duty GP reviews the incident</li>
        </ul>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          If Non-Urgent Side Effect
        </div>
        <ul class="protocol-list">
          <li>Use the <strong>Side Effect Macros</strong> to contact them if needed</li>
          <li><strong>Do NOT</strong> put the order on hold purely for non-urgent side effects</li>
          <li>You can usually still prescribe (assuming no contraindications)</li>
        </ul>
      </div>
      ...
      <!-- Tab 3: Step B - The 4 Prescriber Checks -->
      <div class="protocol-tab-content" data-tab-content="checks">
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Step B: The 4 Prescriber Checks (Core)
        </div>
        <div class="protocol-text">
          These are the detailed checks you must complete for every periodic review:
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Check 1: Weight Loss Checkpoint (Licensing Logic)
        </div>
        <div class="protocol-text">
          A patient should have lost <strong>≥5%</strong> of initial body weight by:
        </div>
        <ul class="protocol-list">
          <li><strong>Mounjaro/Wegovy:</strong> 6 months</li>
          <li><strong>Nevolat:</strong> 4 months</li>
        </ul>
        <div class="protocol-text" style="margin-top: 12px;">
          See the <strong>"Weight Loss Actions"</strong> tab for detailed actions based on % weight loss.
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Check 2: Side Effects Check
        </div>
        <ul class="protocol-list">
          <li>Ensure side effects are appropriately handled</li>
          <li><strong>Urgent</strong> = phone + safety incident (see Step A: Triage)</li>
          <li><strong>Non-urgent</strong> = side effect macros, usually do not hold</li>
        </ul>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Check 3: New Conditions / New Meds Check
        </div>
        <div class="protocol-text">
          If they report anything new:
        </div>
        <ul class="protocol-list">
          <li>Refer to the <strong>Contraindications</strong> section</li>
          <li>If not a contraindication → document it</li>
          <li>If unsure → <strong>escalate to senior prescriber</strong></li>
        </ul>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Check 4: Compliance Check (Weekly Injections)
        </div>
        <div class="protocol-text">
          They should be adhering to the weekly injection schedule.
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">If Non-Compliant:</div>
          <ul class="protocol-list">
            <li>Side effects already dealt with → proceed to next check</li>
            <li>Scheduling issues but weight loss adequate → proceed</li>
            <li>Side effects <strong>not captured earlier</strong> → email to clarify + tag <span class="tag yellow">Pending Customer Response</span></li>
          </ul>
        </div>
      </div>
      ...
      <div class="protocol-card">
        ...
        <div class="checklist-group" style="margin-top: 12px;">
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Periodic review questionnaire completed</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Weight loss percentage calculated</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Side effects reviewed and handled appropriately</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">New conditions/medications checked against contraindications</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Compliance/adherence assessed</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Clinical decision documented with reasoning</span>
          </label>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Urgent Side Effects (Require Immediate Action)
        </div>
        <div class="info-card red" style="margin-top: 8px; margin-bottom: 16px;">
          <div class="info-card-title">Side effects marked with a warning appear in the Urgent queue and require contact from a prescriber within 8 working hours.</div>
        </div>
        <ul class="protocol-list">
          <li><span class="tag red">URGENT</span> Moderate nausea with vomiting</li>
          <li><span class="tag red">URGENT</span> Severe nausea with vomiting and/or diarrhoea</li>
          <li><span class="tag red">URGENT</span> Jaundice (yellowing of your skin or eyes)</li>
          <li><span class="tag red">URGENT</span> Severe abdominal pain</li>
          <li><span class="tag red">URGENT</span> Difficulty breathing or swelling of your face, tongue, or throat</li>
          <li><span class="tag red">URGENT</span> Reduced urine output and/or fluid build-up</li>
          <li><span class="tag red">URGENT</span> Sudden fast heart rate or vision changes</li>
          <li><span class="tag red">URGENT</span> Lumps in your neck or a hoarse voice</li>
          <li><span class="tag red">URGENT</span> Other urgent side effect requiring medical attention</li>
        </ul>
        <div class="protocol-section" style="margin-top: 16px;">
          <div class="protocol-section-title">Required Actions for Urgent Side Effects</div>
          <ul class="protocol-list">
            <li><strong>1.</strong> Telephone contact <strong>within 8 working hours</strong></li>
            <li><strong>2.</strong> If you can't reach them → Add tag <span class="tag yellow">Pending Customer Response</span> and email</li>
            <li><strong>3.</strong> Submit <strong>Patient Safety Incident (PSI)</strong>:
              <ul style="margin-top: 8px; margin-left: 20px;">
                <li>Category: <strong>Clinically Significant Side Effects</strong></li>
                <li>Select correct option for "Did these side effects require medical attention?"</li>
              </ul>
            </li>
            <li><strong>4.</strong> Duty GP will review the incident</li>
          </ul>
        </div>
      </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-text">
          These side effects are common and usually manageable. <strong>Do NOT put the order on hold purely for non-urgent side effects.</strong>
        </div>
        <ul class="protocol-list" style="margin-top: 12px;">
          <li><span class="tag green">Non-urgent</span> Mild nausea, with or without vomiting</li>
          <li><span class="tag green">Non-urgent</span> Constipation</li>
          <li><span class="tag green">Non-urgent</span> Headaches</li>
          <li><span class="tag green">Non-urgent</span> Diarrhoea</li>
          <li><span class="tag green">Non-urgent</span> Heartburn</li>
          <li><span class="tag green">Non-urgent</span> Injection site reactions</li>
          <li><span class="tag green">Non-urgent</span> Other side effect not requiring urgent medical attention</li>
          <li><span class="tag blue">None</span> No, I don't have any side effects</li>
        </ul>
        <div class="protocol-section" style="margin-top: 16px;">
          <div class="protocol-section-title">Actions for Non-Urgent Side Effects</div>
          <ul class="protocol-list">
            <li>Use the <strong>Side Effect Macros</strong> to contact them if needed</li>
            <li><strong>Do NOT</strong> put the order on hold purely for non-urgent side effects</li>
            <li>You can usually still prescribe (assuming no contraindications)</li>
            <li>Document the side effect in notes</li>
          </ul>
        </div>
      </div>
        ...
        <div class="table-wrapper">
          <table class="dose-table">
            ...
            <tr style="background: rgba(239, 68, 68, 0.1);">
              ...
              <td>Phone call + PSI submission</td>
            </tr>
            <tr style="background: rgba(34, 197, 94, 0.1);">
              <td><strong>Non-urgent</strong></td>
              <td>Standard queue</td>
              <td>Standard</td>
              <td>Side effect macros if needed</td>
            </tr>
            <tr style="background: rgba(59, 130, 246, 0.1);">
              <td><strong>None</strong></td>
              <td>Standard queue</td>
              <td>Standard</td>
              <td>Proceed with review</td>
            </tr>
          </table>
        </div>
      </div>
      ...
      <!-- Tab 5: Weight Loss Actions -->
      <div class="protocol-tab-content" data-tab-content="weight-actions">
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Weight Loss ≥5% — PROCEED
        </div>
        <div class="protocol-text">
          Periodic review <strong>not mandatory</strong> (unless they chose to do it).
        </div>
        <ul class="protocol-list">
          <li>Proceed normally with order</li>
          <li>Document weight loss achieved</li>
        </ul>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Weight Loss 3-5% — POSSIBLE CONTINUATION
        </div>
        <div class="protocol-text">
          May continue <strong>if there's a reasonable explanation</strong>:
        </div>
        <ul class="protocol-list">
          <li>Evidence of muscle gain / waist reduction / increased resistance exercise</li>
          <li>Improvements in heart health</li>
          <li>Suboptimal compliance due to scheduling or manageable side effects</li>
          <li>Steady downward trajectory (not flat)</li>
          <li>Not reached maximum dose yet</li>
          <li>Gaps in treatment affecting progress</li>
          <li>Switched GLP-1 and hasn't completed full 4/6 months on the new one</li>
          <li>Side effects limited titration</li>
        </ul>
        <div class="info-card yellow" style="margin-top: 12px;">
          <div class="info-card-title">If You Continue Despite &lt;5%</div>
          <div class="info-card-text">
            <strong>Add a critical note</strong> explaining why you continued + flag future prescribers to monitor weight loss closely.
          </div>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Weight Loss &lt;3% — CONTACT CUSTOMER
        </div>
        <div class="protocol-text">
          <strong>Contact customer for a detailed assessment:</strong>
        </div>
        <ul class="protocol-list">
          <li>Tag order with <span class="tag yellow">Pending Customer Response</span></li>
          <li>Gather information: diet, activity, adherence, gaps, side effects, new conditions/meds</li>
        </ul>
        <div class="protocol-section">
          <div class="protocol-section-title">Call Script for 0-3% Weight Loss</div>
          <ul class="protocol-list">
            <li>ID check (DOB + postcode)</li>
            <li>Confirm current height/weight</li>
            <li>Diet pattern and activity (with time categories)</li>
            <li>Adherence, gaps, side effects</li>
            <li>Any other reason for not losing weight</li>
            <li>Whether they'd consider an oral option like <strong>orlistat</strong> if GLP-1 can't continue</li>
          </ul>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Weight Unchanged or Increased — STOP
        </div>
        <div class="protocol-text">
          If weight remains <strong>unchanged or has increased</strong> after 4/6 months:
        </div>
        <ul class="protocol-list">
          <li>Tell patient the medication is <strong>no longer suitable</strong> under licensing requirements</li>
          <li>Treatment <strong>cannot be continued</strong></li>
          <li><strong>Cancel order</strong></li>
        </ul>
        <div class="info-card red" style="margin-top: 12px;">
          <div class="info-card-title">Documentation Template</div>
          <div class="info-card-text">
            "Periodic review completed. Weight unchanged/increased after [4/6] months. Per product licence requirements, treatment cannot be continued. Order cancelled. Patient advised to discuss alternative weight management options with GP."
          </div>
        </div>
      </div>
      ...
      <!-- Tab 5: Exemptions -->
      <div class="protocol-tab-content" data-tab-content="exemptions">
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Exemption A: Transfers from Other Providers
        </div>
        <div class="protocol-text">
          A transferred patient can be <strong>exempt</strong> from immediate periodic review if they meet <strong>ALL 3 criteria</strong>:
        </div>
        <div class="checklist-group" style="margin-top: 12px;">
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Start date on previous use form is <strong>&gt;7 months ago</strong></span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text"><strong>No side effects</strong> reported on previous use form</span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Latest weight is <strong>&gt;5% less</strong> than starting weight on previous upload form</span>
          </label>
        </div>
        <div class="protocol-text" style="margin-top: 12px;">
          <strong>Also:</strong> Breaks due to <strong>pregnancy</strong> are exempt.
        </div>
        <div class="info-card blue" style="margin-top: 12px;">
          <div class="info-card-title">If Not All 3 Criteria Met</div>
          <div class="info-card-text">
            Invite to periodic review once they reach the 4/6-month mark. Some may be asked earlier if side effects or poor weight loss reported.
          </div>
        </div>
      </div>
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          Exemption B: Gaps in Treatment (Manual Exemption)
        </div>
        <div class="protocol-text">
          The system currently <strong>doesn't account for gaps</strong>, so prescribers may need to <strong>manually exempt</strong>.
        </div>
        <div class="protocol-text" style="margin-top: 8px;">
          Manual exemption can apply if <strong>ALL 3 criteria</strong> are met:
        </div>
        <div class="checklist-group" style="margin-top: 12px;">
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Treatment gap <strong>&gt;12 weeks</strong></span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Restarted at the <strong>lowest dose</strong></span>
          </label>
          <label class="checklist-item">
            <input type="checkbox" />
            <span class="checkmark"></span>
            <span class="checklist-text">Since restarting, they've had <strong>&lt;3 months</strong> treatment</span>
          </label>
        </div>
        <div class="info-card yellow" style="margin-top: 12px;">
          <div class="info-card-title">If Not All 3 Criteria Met</div>
          <div class="info-card-text">
            They complete periodic review at the standard 4/6-month mark from <strong>first ever</strong> treatment start date.
          </div>
        </div>
      </div>
      ...
      <!-- Tab 6: Closing Review -->
      <div class="protocol-tab-content" data-tab-content="closing">
      ...
      <div class="protocol-card">
        <div class="protocol-title">
          ...
          How to Close the Review Properly
        </div>
        <div class="protocol-text">
          Once satisfied with all checks, complete these steps:
        </div>
        <ul class="protocol-list">
          <li><strong>1.</strong> Document in Notes: <strong>"Complete In depth review at 4/6 months"</strong></li>
          <li><strong>2.</strong> Click <strong>"Mark Resolved"</strong></li>
          <li><strong>3.</strong> Proceed with reviewing their next order in line with standard SOP</li>
        </ul>
      </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          <div class="protocol-section-title">≥5% Weight Loss (Approve)</div>
          <div class="info-card green">
            <div class="info-card-text">
              "Complete in-depth review at 6 months. Weight loss: X% (starting: Xkg, current: Xkg). Good response to treatment. No new contraindications. Safe to continue."
            </div>
          </div>
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">3-5% Weight Loss (Continue with Reasoning)</div>
          <div class="info-card yellow">
            <div class="info-card-text">
              "Complete in-depth review at 6 months. Weight loss: X% (suboptimal). [Reason for continuation: e.g., recent illness, gaps in treatment, not yet at max dose]. Added critical note. Monitor closely at next review."
            </div>
          </div>
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">&lt;3% Weight Loss - Pending Response</div>
          <div class="info-card orange">
            <div class="info-card-text">
              "Periodic review at 6 months. Weight loss: X% (insufficient). Contacted patient for detailed assessment. Tagged Pending Customer Response. Awaiting information on diet, activity, adherence before final decision."
            </div>
          </div>
        </div>
        <div class="protocol-section">
          <div class="protocol-section-title">Unchanged/Increased Weight (Stop)</div>
          <div class="info-card red">
            <div class="info-card-text">
              "Periodic review at 6 months. Weight unchanged/increased. Per product licence requirements, treatment cannot be continued. Order cancelled. Patient advised to discuss alternative weight management options with GP."
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="protocol-tab-content" data-tab-content="cheatsheet">
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-text">
          When you open a periodic review, ask yourself <strong>in this order</strong>:
        </div>
      </div>

      <div class="protocol-card" style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.1) 0%, rgba(239, 68, 68, 0.05) 100%);">
        <div class="protocol-section">
          <div class="protocol-section-title">1. Urgent Side Effect Flag?</div>
          <ul class="protocol-list">
            <li><strong>Yes →</strong> Urgent queue rules + phone within 8 working hours + safety incident</li>
            <li><strong>No →</strong> Proceed to next check</li>
          </ul>
        </div>
      </div>

      <div class="protocol-card" style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.1) 0%, rgba(34, 197, 94, 0.05) 100%);">
        <div class="protocol-section">
          <div class="protocol-section-title">2. Weight Loss % at the Mark?</div>
          <div class="table-wrapper">
            <table class="dose-table">
              <tr>
                <th>Weight Loss</th>
              ...
              <tr style="background: rgba(34, 197, 94, 0.15);">
                ...
                <td><span class="tag green">PROCEED</span></td>
              </tr>
              <tr style="background: rgba(251, 191, 36, 0.15);">
                <td><strong>3-5%</strong></td>
                <td><span class="tag yellow">Continue only if clear justification; add critical note</span></td>
              </tr>
              <tr style="background: rgba(249, 115, 22, 0.15);">
                <td><strong>&lt;3%</strong></td>
                <td><span class="tag orange">Contact + Pending Response</span></td>
              </tr>
              <tr style="background: rgba(239, 68, 68, 0.15);">
                <td><strong>Unchanged/Increased</strong></td>
                <td><span class="tag red">STOP + Cancel</span></td>
              </tr>
            </table>
          </div>
        </div>
      ...
      <div class="protocol-card" style="background: linear-gradient(135deg, rgba(139, 92, 246, 0.1) 0%, rgba(139, 92, 246, 0.05) 100%);">
        <div class="protocol-section">
          <div class="protocol-section-title">3. Any New Meds/Conditions?</div>
          <ul class="protocol-list">
            <li>Check Contraindications section</li>
            <li><strong>If unsure →</strong> Senior prescriber escalation</li>
          </ul>
        </div>
      </div>

      <div class="protocol-card" style="background: linear-gradient(135deg, rgba(59, 130, 246, 0.1) 0%, rgba(59, 130, 246, 0.05) 100%);">
        <div class="protocol-section">
          <div class="protocol-section-title">4. Adherence OK?</div>
          <ul class="protocol-list">
            <li><strong>If not, why?</strong> (side effects vs scheduling vs other)</li>
            <li>Side effects already dealt with → proceed</li>
            <li>Scheduling issues but adequate weight loss → proceed</li>
            <li>Unresolved issues → email + Pending Customer Response</li>
          </ul>
        </div>
      </div>
      ...
      <div class="protocol-card">
        ...
        <div class="protocol-section">
          <div class="protocol-section-title">Why This Matters for Reviews</div>
          <ul class="protocol-list">
            <li>Treatment history may span both platforms</li>
            <li>Starting weight and treatment start date may be from Levity account</li>
            <li>Previous review information may be in Levity records</li>
            <li>Total treatment duration calculation should include both</li>
          </ul>
        </div>
        <div class="protocol-section">
          ...
          <ul class="protocol-list">
            ...
            <li>Calculate treatment duration from earliest start date across both platforms</li>
          </ul>
        </div>
...
MedExpress Clinical Team</pre>
        ...
        <!-- Macro 13 - PUE/Transfer Evidence Requirements -->
        <div class="macro-card" id="macro-13">
          <div class="macro-header" onclick="this.parentElement.classList.toggle('expanded')">
            <div class="macro-title">
              <span class="macro-number">Macro 13</span>
              <span class="macro-name">Previous Use Evidence And Previous BMI Verification Photo</span>
            </div>
            <div class="macro-tags">
        ...
        <div class="macro-card" id="macro-14">
          <div class="macro-header" onclick="this.parentElement.classList.toggle('expanded')">
            <div class="macro-title">
              ...
              <span class="macro-name">Previous Use Evidence</span>
            </div>
            <div class="macro-tags">
              <span class="tag orange">Transfer Patients</span>
            </div>
        ...
        <div class="macro-card" id="macro-15">
          <div class="macro-header" onclick="this.parentElement.classList.toggle('expanded')">
            <div class="macro-title">
              ...
              <span class="macro-name">Previous BMI Photo (StartingBMI)</span>
            </div>
            <div class="macro-tags">
              <span class="tag orange">Transfer Patients</span>
              <span class="tag blue">Below Licence BMI</span>
...
Kind regards,
MedExpress Clinical Team Clinical Team</pre>
            </div>
            <div class="macro-decision-guide">
              <div class="decision-section reject">
        ...
        <!-- Macro 32 - Combined PUE & Previous Weight Photo Request -->
        <div class="macro-card" id="macro-32">
          <div class="macro-header" onclick="this.parentElement.classList.toggle('expanded')">
            <div class="macro-title">
              <span class="macro-number">Macro 32</span>
              <span class="macro-name">Combined PUE & Previous Weight Photo Request</span>
            </div>
            <div class="macro-tags">
              <span class="tag orange">Transfer Patients</span>
              <span class="tag purple">Below Licence BMI</span>
            </div>
            <svg class="macro-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polyline points="6 9 12 15 18 9"/>
            </svg>
          ...
          <div class="macro-content">
            <div class="macro-description">
              <strong>Use when:</strong> Transfer patient needs to provide BOTH proof of previous use (PUE) AND a previous weight verification photo. This is the combined macro for below-licence BMI transfer patients who need both pieces of evidence.
            </div>
            <div class="macro-template">
              <div class="macro-template-header">
                <span>Email Template</span>
...
Thank you for your order with MedExpress. As you are transferring from another provider and your current BMI is below the standard licensing threshold, we need to verify both your previous treatment and that you met the BMI criteria when you first started GLP-1 therapy.

Please provide the following TWO items:

1. PROOF OF PREVIOUS USE (PUE)

Evidence of your previous GLP-1 treatment must clearly show ALL of the following:
• Patient name or email – your full name (not a nickname) or the email address used with the previous provider
• Medication and dose – the specific GLP-1 medication and dosage you were prescribed
• Date – the order date, prescription date, or dispatch date
• From a regulated provider – such as a pharmacy, clinic, or healthcare service

Acceptable evidence includes: Prescription, dispensing label, or dispatch notification.
Order confirmations alone are NOT accepted.
...
• Shows you full-length or nearly full-length in fitted clothing
• Is well lit and clear enough for us to see your body shape
• Demonstrates that you met the licensed BMI threshold at that time

Please reply to this email with both pieces of evidence attached. We cannot proceed with your order until we receive both items.
...
MedExpress Clinical Team</pre>
            ...
            <div class="macro-info-box">
              <div class="info-title">📋 Required Actions</div>
              <p><strong>After sending this macro:</strong></p>
              <ul>
                <li>Add tag: <span style="display: inline-block; padding: 3px 8px; background: var(--orange-bg); color: var(--orange); border-radius: 4px; font-size: 11px; font-weight: 600;">Pending Customer Response</span></li>
                <li>Remove tag: <span style="display: inline-block; padding: 3px 8px; background: var(--accent-bg); color: var(--accent); border-radius: 4px; font-size: 11px; font-weight: 600;">prescriber review</span></li>
              </ul>
            </div>
          </div>
        </div>
...
Proceed according to standard prescription SOP. Hold only if specific information is required about a condition/medication.</pre>
      ...
      <div class="protocol-page">
          ...
          <div class="table-wrapper">
            ...
            <tr>
              ...
              <td>
                ...
                <span style="font-size: 12px;">Use <a href="#macro-13" onclick="navigateToMacro(13); return false;" class="tag blue">Macro 13: PUE &amp; BMI Photo</a> or <a href="#macro-15" onclick="navigateToMacro(15); return false;" class="tag blue">Macro 15: Previous BMI Photo</a></span>
              </td>
            </tr>
            </table>
          </div>
          ...
          <div class="info-card orange" style="margin-top: 20px;">
            <div style="display: flex; align-items: flex-start; gap: 10px;">
              ...
              <div>
                ...
                3. Send <a href="#macro-13" onclick="navigateToMacro(13); return false;" class="tag blue" style="margin: 2px 0;">Macro 13: PUE &amp; BMI Photo</a> or <a href="#macro-15" onclick="navigateToMacro(15); return false;" class="tag blue" style="margin: 2px 0;">Macro 15: Previous BMI Photo</a><br>
        ...
        <div class="protocol-tab-content" data-tab-content="above">
          ...
          <div class="protocol-card" style="margin-top: 20px;">
            ...
            <ul class="protocol-list" style="margin-top: 12px;">
              <li>Send <a href="#macro-14" onclick="navigateToMacro(14); return false;" class="tag blue">Macro 14: Previous Use Evidence</a></li>
          ...
          <div class="protocol-card" style="margin-top: 20px;">
            ...
            <div style="display: grid; gap: 12px; margin-top: 16px;">
              ...
              <div style="display: flex; align-items: start; gap: 12px; padding: 12px; background: var(--bg-elevated); border-radius: 8px;">
                ...
                <div>Send <a href="#macro-15" onclick="navigateToMacro(15); return false;" class="tag blue">Macro 15: Previous BMI Photo</a></div>
              </div>
            </div>
  ...
  getNevolatExclusionContent() {
    return `
      <div class="protocol-page">
        <h1 style="font-size: 24px; font-weight: 700; margin-bottom: 16px;">Nevolat Specific Exclusion</h1>
        <p style="color: var(--text-secondary); margin-bottom: 24px;">Critical exclusion criteria for Nevolat prescribing - Review before prescribing Nevolat</p>
        ...
        <div class="info-card red" style="margin-bottom: 24px;">
          <div style="display: flex; align-items: center; gap: 10px;">
            ...
            <div>
              <strong>CRITICAL:</strong> Patients with liver disease or thyroid conditions must NOT be prescribed Nevolat. These patients will only see Mounjaro and Wegovy as available options on the Choose Treatment Page.
            </div>
          </div>
        </div>
        ...
        <div style="display: grid; gap: 16px; margin-bottom: 24px;">
          <div class="info-card" style="border-left: 4px solid var(--danger);">
            <h3 style="font-size: 16px; font-weight: 700; margin-bottom: 8px; color: var(--danger);">Liver Disease</h3>
            <ul style="margin: 0; padding-left: 20px; color: var(--text-secondary);">
              <li>Any current or history of liver disease</li>
              <li>Elevated liver enzymes (ALT/AST)</li>
              <li>Hepatitis (any type)</li>
              <li>Cirrhosis or fatty liver disease</li>
              <li>Liver failure or transplant history</li>
            </ul>
            <div style="margin-top: 12px; padding: 10px; background: rgba(239, 68, 68, 0.1); border-radius: 8px;">
              <strong style="color: var(--danger);">Action:</strong> <span style="color: var(--text-primary);">Do NOT prescribe Nevolat. Offer Mounjaro or Wegovy instead.</span>
            </div>
          </div>

          <div class="info-card" style="border-left: 4px solid var(--danger);">
            <h3 style="font-size: 16px; font-weight: 700; margin-bottom: 8px; color: var(--danger);">Thyroid Conditions</h3>
            <ul style="margin: 0; padding-left: 20px; color: var(--text-secondary);">
              <li>Any current or history of thyroid disease</li>
              <li>Hyperthyroidism or hypothyroidism</li>
              <li>Thyroid nodules or goiter</li>
              <li>Thyroid cancer or family history of MTC</li>
        ...
        <div class="info-card blue" style="margin-bottom: 24px;">
          <div style="display: flex; align-items: flex-start; gap: 10px;">
            ...
            <div>
              <strong>Choose Treatment Page (CTP):</strong> The system automatically hides Nevolat from patients who have disclosed liver or thyroid conditions during their consultation. These patients will only see Mounjaro and Wegovy as available medication options.
            </div>
          </div>
        </div>
        ...
        <div style="display: grid; gap: 12px; margin-bottom: 24px;">
          <div class="checklist-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--bg-elevated); border-radius: 8px; border: 1px solid var(--border-color);">
            <div style="width: 24px; height: 24px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
              <span style="color: white; font-weight: 700; font-size: 12px;">1</span>
            </div>
            <span>Confirm patient has NO liver disease history or current liver conditions</span>
          </div>
          <div class="checklist-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--bg-elevated); border-radius: 8px; border: 1px solid var(--border-color);">
            <div style="width: 24px; height: 24px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
              <span style="color: white; font-weight: 700; font-size: 12px;">2</span>
            </div>
            <span>Confirm patient has NO thyroid disease history or current thyroid conditions</span>
          </div>
          <div class="checklist-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--bg-elevated); border-radius: 8px; border: 1px solid var(--border-color);">
            <div style="width: 24px; height: 24px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
              <span style="color: white; font-weight: 700; font-size: 12px;">3</span>
            </div>
            <span>Verify SCR shows no undisclosed liver or thyroid medications</span>
          </div>
          <div class="checklist-item" style="display: flex; align-items: center; gap: 12px; padding: 12px 16px; background: var(--bg-elevated); border-radius: 8px; border: 1px solid var(--border-color);">
            <div style="width: 24px; height: 24px; border-radius: 50%; background: var(--primary); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
              <span style="color: white; font-weight: 700; font-size: 12px;">4</span>
            </div>
            <span>If ANY doubt exists, offer alternative medication (Mounjaro/Wegovy)</span>
          </div>
        </div>
        ...
        <div class="info-card green" style="margin-bottom: 24px;">
          <div style="display: flex; align-items: flex-start; gap: 10px;">
            ...
            <div>
              <strong>Safe to Prescribe Nevolat:</strong> Patient has confirmed NO liver conditions AND NO thyroid conditions in their consultation responses, AND SCR shows no liver/thyroid medications.
            </div>
          </div>
        </div>
        ...
        <div class="info-card" style="margin-bottom: 16px;">
          <p style="margin: 0 0 12px 0; color: var(--text-secondary);">When prescribing Nevolat, document in clinical notes:</p>
          <ul style="margin: 0; padding-left: 20px; color: var(--text-secondary);">
            <li>Confirmed no liver disease history</li>
            <li>Confirmed no thyroid disease history</li>
            <li>SCR reviewed - no liver/thyroid medications identified</li>
            <li>Patient suitable for Nevolat prescription</li>
          </ul>
        </div>
      </div>
    `;
  ...
  getBMICalculatorContent() {
    return `
      <div class="protocol-page">
        <h1 style="font-size: 24px; font-weight: 700; margin-bottom: 8px;">BMI Calculator</h1>
        <p style="color: var(--text-secondary); margin-bottom: 24px;">Medical-grade calculation with NICE ethnicity-adjusted thresholds</p>
        ...
        <div class="bmi-calculator-wrapper">
          <div class="bmi-calculator-container">
            ...
            <div class="bmi-input-card">
              <!-- Unit Toggle -->
              <div class="bmi-field">
                <label class="bmi-label">Unit System</label>
                <div class="bmi-units-toggle">
                  <button id="metricBtn" class="bmi-unit-btn active">Metric (cm/kg)</button>
                  <button id="imperialBtn" class="bmi-unit-btn">Imperial (ft/st)</button>
                </div>
              </div>

              <!-- Sex -->
              <div class="bmi-field">
                <label class="bmi-label">Sex</label>
                <div class="bmi-radio-group">
                  <div class="bmi-radio-option active" id="maleOption">
                    <input type="radio" name="gender" value="male" id="genderMale" checked>
                    <div class="bmi-radio-dot"></div>
                    <span>Male</span>
                  </div>
                  <div class="bmi-radio-option" id="femaleOption">
              ...
              <!-- Ethnicity -->
              <div class="bmi-field">
                <label class="bmi-label">Ethnicity</label>
                <select id="ethnicitySelect" class="bmi-select">
                  <option value="">Select ethnicity...</option>
                  <optgroup label="Standard Thresholds">
                    <option value="white">White</option>
                    <option value="mixed">Mixed</option>
                    <option value="other">Other</option>
                  </optgroup>
                  <optgroup label="NICE Adjusted (Lower Thresholds)">
              ...
              <div class="bmi-field">
                ...
                <div id="heightMetric" class="bmi-input-wrapper">
                  <input type="number" id="heightCm" class="bmi-input" placeholder="175">
                  <span class="bmi-input-unit">cm</span>
                </div>
                <div id="heightImperial" class="bmi-input-row" style="display: none;">
                  <div class="bmi-input-wrapper" style="flex: 1;">
                    <input type="number" id="heightFt" class="bmi-input" placeholder="5">
                    <span class="bmi-input-unit">ft</span>
                  </div>
                  <div class="bmi-input-wrapper" style="flex: 1;">
                    <input type="number" id="heightIn" class="bmi-input" placeholder="10">
                    <span class="bmi-input-unit">in</span>
                  </div>
                </div>
              </div>
              ...
              <div class="bmi-field">
                ...
                <div id="weightMetric" class="bmi-input-wrapper">
                  <input type="number" id="weightKg" class="bmi-input" placeholder="75">
                  <span class="bmi-input-unit">kg</span>
                </div>
                <div id="weightImperial" class="bmi-input-row" style="display: none;">
                  <div class="bmi-input-wrapper" style="flex: 1;">
                    <input type="number" id="weightSt" class="bmi-input" placeholder="11">
                    <span class="bmi-input-unit">st</span>
                  </div>
                  <div class="bmi-input-wrapper" style="flex: 1;">
                    <input type="number" id="weightLbs" class="bmi-input" placeholder="10">
                    <span class="bmi-input-unit">lbs</span>
                  </div>
                </div>
              </div>

              <!-- Calculate Button -->
              <button id="calculateBmiBtn" class="bmi-calc-btn">
            ...
            <!-- Right: Results -->
            <div class="bmi-results-column">
              <div id="bmiResults" class="bmi-result-card" style="display: none;">
                <div class="bmi-result-label">Body Mass Index</div>
                <div id="bmiValue" class="bmi-result-value">--</div>
                <div class="bmi-result-unit">kg/m²</div>
                <div id="bmiCategoryBadge" class="bmi-category-badge">--</div>
                
                <div class="bmi-stats-row">
                  <div class="bmi-stat">
                    <span class="bmi-stat-label">Height</span>
                    <span id="resultHeight" class="bmi-stat-value">--</span>
                  </div>
                  <div class="bmi-stat">
                    <span class="bmi-stat-label">Weight</span>
                    <span id="resultWeight" class="bmi-stat-value">--</span>
                  </div>
                </div>

                <div id="ethnicityNote" class="bmi-ethnicity-note" style="display: none;">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width: 16px; height: 16px; flex-shrink: 0;">
                    <circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/>
                  </svg>
                  <span id="ethnicityNoteText"></span>
                </div>

                <div id="glp1Eligibility" class="bmi-eligibility">
                  <div class="bmi-eligibility-title">GLP-1 Eligibility</div>
                  <div id="eligibilityContent" class="bmi-eligibility-content"></div>
                </div>
              </div>

              <!-- Placeholder -->
              <div id="bmiPlaceholder" class="bmi-placeholder">
                ...
                <p>Enter details to calculate BMI</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Reference Section -->
        <details class="bmi-details-section" style="margin-top: 24px;">
          <summary style="background: var(--bg-elevated); border-radius: 12px; padding: 14px 20px; border: 1px solid var(--border); cursor: pointer; font-weight: 700; font-size: 14px; display: flex; align-items: center; gap: 10px; list-style: none;">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="width: 18px; height: 18px;" class="details-chevron">
              <polyline points="6 9 12 15 18 9"/>
            </svg>
            BMI Reference & GLP-1 Criteria
          </summary>
          <div style="padding: 20px; background: var(--bg-card); border: 1px solid var(--border); border-top: none; border-radius: 0 0 16px 16px; box-shadow: var(--shadow-sm);">
            <div style="overflow-x: auto; margin-bottom: 24px;">
              <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                <thead>
                  <tr style="text-align: left; border-bottom: 2px solid var(--border);">
                    <th style="padding: 10px; color: var(--text-secondary);">Category</th>
                    <th style="padding: 10px; text-align: center; color: var(--text-secondary);">Standard</th>
                    <th style="padding: 10px; text-align: center; color: var(--text-secondary);">NICE*</th>
                  </tr>
                </thead>
                <tbody>
                  <tr style="border-bottom: 1px solid var(--border);">
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #3B82F6; margin-right: 8px;"></span>Underweight</td>
                    <td style="padding: 10px; text-align: center;">&lt; 18.5</td>
                    <td style="padding: 10px; text-align: center;">&lt; 18.5</td>
                  </tr>
                  <tr style="border-bottom: 1px solid var(--border);">
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #22C55E; margin-right: 8px;"></span>Healthy</td>
                    <td style="padding: 10px; text-align: center;">18.5 - 24.9</td>
                    <td style="padding: 10px; text-align: center;">18.5 - 22.9</td>
                  </tr>
                  <tr style="border-bottom: 1px solid var(--border);">
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #F59E0B; margin-right: 8px;"></span>Overweight</td>
                    <td style="padding: 10px; text-align: center;">25 - 29.9</td>
                    <td style="padding: 10px; text-align: center;">23 - 27.4</td>
                  </tr>
                  <tr style="border-bottom: 1px solid var(--border);">
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #F97316; margin-right: 8px;"></span>Obese I</td>
                    <td style="padding: 10px; text-align: center;">30 - 34.9</td>
                    <td style="padding: 10px; text-align: center;">27.5 - 32.4</td>
                  </tr>
                  <tr style="border-bottom: 1px solid var(--border);">
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #EF4444; margin-right: 8px;"></span>Obese II</td>
                    <td style="padding: 10px; text-align: center;">35 - 39.9</td>
                    <td style="padding: 10px; text-align: center;">32.5 - 37.4</td>
                  </tr>
                  <tr>
                    <td style="padding: 10px;"><span style="display: inline-block; width: 10px; height: 10px; border-radius: 2px; background: #991B1B; margin-right: 8px;"></span>Obese III</td>
                    <td style="padding: 10px; text-align: center;">&ge; 40</td>
                    <td style="padding: 10px; text-align: center;">&ge; 37.5</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p style="font-size: 12px; color: var(--text-muted); margin-bottom: 20px;">*NICE adjusted thresholds for South Asian, Chinese, other Asian, Middle Eastern, Black African, and African-Caribbean ethnicities.</p>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
              <div style="background: rgba(34, 197, 94, 0.08); border: 1px solid rgba(34, 197, 94, 0.15); border-radius: 12px; padding: 16px;">
                <div style="font-weight: 800; font-size: 13px; color: #22C55E; margin-bottom: 8px; text-transform: uppercase;">Eligible for GLP-1</div>
                <ul style="margin: 0; padding-left: 20px; font-size: 12px; color: var(--text-secondary); line-height: 1.6;">
                  <li>BMI &ge; 30 (or &ge; 27.5 NICE)</li>
                  <li>BMI &ge; 27 (or &ge; 25 NICE) + comorbidity</li>
                </ul>
              </div>
              <div style="background: rgba(245, 158, 11, 0.08); border: 1px solid rgba(245, 158, 11, 0.15); border-radius: 12px; padding: 16px;">
                <div style="font-weight: 800; font-size: 13px; color: #F59E0B; margin-bottom: 8px; text-transform: uppercase;">Comorbidities</div>
                <ul style="margin: 0; padding-left: 20px; font-size: 12px; color: var(--text-secondary); line-height: 1.6;">
                  <li>Type 2 diabetes, Hypertension</li>
                  <li>Dyslipidaemia, Sleep apnoea</li>
                  <li>CVD, NAFLD</li>
                </ul>
              </div>
            </div>
  ...
  attachEventListeners(pageId) {
    ...
    if (
      ...
      // Handle pending tab navigation from subnav
      if (NavigationManager.pendingTab) {
        const pendingTabId = NavigationManager.pendingTab;
        NavigationManager.pendingTab = null;
        
        const targetTab = document.querySelector(`.protocol-tab[data-tab="${pendingTabId}"]`);
        if (targetTab) {
          tabs.forEach((t) => t.classList.remove("active"));
          tabContents.forEach((content) => content.classList.remove("active"));
          
          targetTab.classList.add("active");
          const activeContent = document.querySelector(`[data-tab-content="${pendingTabId}"]`);
          if (activeContent) {
            activeContent.classList.add("active");
          }
          ...
          document.querySelectorAll(".subnav-item").forEach((item) => {
            if (item.dataset.page === pageId && item.dataset.tab === pendingTabId) {
    ...
    if (pageId === "bmi-calculator") {
      ...
      if (metricBtn && imperialBtn) {
        metricBtn.addEventListener("click", () => {
          ...
          if (heightMetric) heightMetric.style.display = "flex";
          if (heightImperial) heightImperial.style.display = "none";
          if (weightMetric) weightMetric.style.display = "flex";
          if (weightImperial) weightImperial.style.display = "none";
        });
        ...
        imperialBtn.addEventListener("click", () => {
          ...
          if (heightMetric) heightMetric.style.display = "none";
          if (heightImperial) heightImperial.style.display = "flex";
          if (weightMetric) weightMetric.style.display = "none";
          if (weightImperial) weightImperial.style.display = "flex";
        });
      }

      // Sex option selection handlers
      ...
      if (calculateBtn) {
        calculateBtn.addEventListener("click", () => {
          ...
          if (currentUnit === "metric") {
            ...
            const weightKgVal = parseFloat(document.getElementById("weightKg")?.value) || 0;
            ...
            weightKg = (st * 6.35029) + (lbs * 0.453592);
            heightDisplay = `${ft}ft ${inches}in`;
            weightDisplay = `${st}st ${lbs}lbs`;
          }
          ...
          const niceAdjusted = ["south-asian", "chinese", "other-asian", "black-african", "african-caribbean", "middle-eastern"].includes(ethnicity);

          // Determine BMI category based on ethnicity
          let category, categoryColor, categoryBg;
          
          if (niceAdjusted) {
            // NICE adjusted thresholds
            if (bmi < 18.5) {
              ...
              categoryBg = "rgba(249, 115, 22, 0.15)";
            } else if (bmi < 37.5) {
              ...
              categoryBg = "rgba(153, 27, 27, 0.15)";
            }
          } else {
          ...
          const resultHeightEl = document.getElementById("resultHeight");
          const resultWeightEl = document.getElementById("resultWeight");
          const ethnicityNote = document.getElementById("ethnicityNote");
          const ethnicityNoteText = document.getElementById("ethnicityNoteText");
          ...
          if (bmiValueEl) {
            ...
            bmiValueEl.style.color = categoryColor;
          }
          if (categoryBadge) {
            categoryBadge.textContent = category;
            categoryBadge.style.background = categoryBg;
            categoryBadge.style.color = categoryColor;
            categoryBadge.style.border = `1px solid ${categoryColor}`;
          }
          if (resultHeightEl) resultHeightEl.textContent = heightDisplay;
          if (resultWeightEl) resultWeightEl.textContent = weightDisplay;

          // Show ethnicity note if applicable
          if (ethnicityNote && ethnicityNoteText) {
            if (niceAdjusted) {
              ethnicityNote.style.display = "flex";
              ethnicityNoteText.textContent = "NICE-adjusted BMI thresholds applied. This ethnicity has increased metabolic risk at lower BMI values. Classification uses lower thresholds per NICE PH46 guidance.";
            } else if (ethnicity) {
              ethnicityNote.style.display = "none";
            } else {
              ethnicityNote.style.display = "flex";
              ethnicityNoteText.textContent = "No ethnicity selected. Please select ethnicity for accurate threshold classification.";
            }
          }

          // Eligibility Logic
          if (eligibilityContent) {
            const obeseThreshold = niceAdjusted ? 27.5 : 30;
            const overweightComorbidThreshold = niceAdjusted ? 25 : 27;
            ...
            if (bmi >= obeseThreshold) {
              eligibilityHtml = `
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                  ...
                  <strong style="color: #22C55E;">Eligible for GLP-1</strong>
                </div>
                <p style="margin: 0;">BMI of ${bmiRounded.toFixed(1)} meets the threshold of ≥${obeseThreshold} kg/m² for GLP-1 treatment eligibility${niceAdjusted ? " (NICE-adjusted)" : ""}.</p>
              `;
            } else if (bmi >= overweightComorbidThreshold) {
              eligibilityHtml = `
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                  ...
                  <strong style="color: #F59E0B;">Conditionally Eligible</strong>
                </div>
                <p style="margin: 0;">BMI of ${bmiRounded.toFixed(1)} is between ${overweightComorbidThreshold}-${obeseThreshold} kg/m². Patient may be eligible for GLP-1 if they have at least one weight-related comorbidity (T2DM, hypertension, dyslipidaemia, sleep apnoea, CVD, or NAFLD).</p>
              `;
            } else {
              eligibilityHtml = `
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                  <svg viewBox="0 0 24 24" fill="none" stroke="#EF4444" stroke-width="2" style="width: 20px; height: 20px; flex-shrink: 0;">
                    <circle cx="12" cy="12" r="10"/>
                    <line x1="15" y1="9" x2="9" y2="15"/>
                    <line x1="9" y1="9" x2="15" y2="15"/>
                  </svg>
                  <strong style="color: #EF4444;">Not Eligible</strong>
                </div>
                <p style="margin: 0;">BMI of ${bmiRounded.toFixed(1)} is below the minimum threshold of ${overweightComorbidThreshold} kg/m² for GLP-1 treatment eligibility${niceAdjusted ? " (NICE-adjusted)" : ""}.</p>
              `;
            }
            
            eligibilityContent.innerHTML = eligibilityHtml;
          }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Mobile Bottom Navigation Handler - must be after NavigationManager.init()
  ...
  function openMobileMenu() {
    if (mobileMenuOverlay && mobileMenuSheet) {
  ...
  if (mobileBottomNav) {
    ...
    mobileNavItems.forEach(item => {
      item.addEventListener("click", function() {
        ...
        const pageId = navMappings[navTarget];
    ...
    // Wrap NavigationManager.goToPage to update mobile nav active states
    const originalGoToPage = NavigationManager.goToPage.bind(NavigationManager);
    NavigationManager.goToPage = function(pageId) {
      originalGoToPage(pageId);
      
      let activeNav = "dashboard";
      if (pageId === "checklists" || (pageId && pageId.includes("checklist"))) {
        activeNav = "checklists";
      } else if (pageId && pageId.startsWith("proto-")) {
        activeNav = "protocols";
      } else if (["definitions", "consultation", "contraindications", "nevolat-exclusion", "titration-guide", "rejections", "tags", "macros", "transfer", "bmi-calculator"].includes(pageId)) {
        activeNav = "more";
      }
...
const KeyboardShortcuts = {
  ...
  init() {
    this.overlay = document.getElementById('keyboardHelpOverlay');
  ...
  handleKeydown(e) {
    ...
    // Don't handle other shortcuts while typing (except Escape)
    if (isTyping) return;
    ...
    if (e.key.toLowerCase() === 't') {
      ...
      ToastManager.show('Theme toggled', 'info', 1500);
      return;
    }
    ...
    if (/^[1-9]$/.test(e.key) && this.shortcuts[e.key]) {
      e.preventDefault();
      NavigationManager.goToPage(this.shortcuts[e.key]);
      return;
    }
  },
...
const FavoritesManager = {
  ...
  pageInfo: {
    'dashboard': { title: 'Dashboard', category: 'Main', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>' },
    'checklists': { title: 'Checklists', category: 'Main', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg>' },
    'proto-core': { title: 'Core Checks', category: 'Protocols', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/></svg>' },
    'proto-scr': { title: 'SCR Screening', category: 'Protocols', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l7 4v6c0 5-3 9-7 10-4-1-7-5-7-10V6l7-4z"/></svg>' },
    'proto-pue': { title: 'Previous Use Evidence', category: 'Protocols', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>' },
    'proto-titration': { title: 'Titration Protocol', category: 'Protocols', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20V10M18 20V4M6 20v-4"/></svg>' },
    'definitions': { title: 'Definitions', category: 'Reference', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/></svg>' },
    'macros': { title: 'Email Macros', category: 'Reference', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 7l-10 7L2 7"/></svg>' },
    'contraindications': { title: 'Contraindications', category: 'Reference', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg>' },
    'bmi-calculator': { title: 'BMI Calculator', category: 'Tools', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="4" y="2" width="16" height="20" rx="2"/><line x1="8" y1="6" x2="16" y2="6"/></svg>' },
    'titration-guide': { title: 'Titration Guide', category: 'Reference', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20V10M18 20V4M6 20v-4"/></svg>' },
    'transfer': { title: 'Transfer Patients', category: 'Main', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 014-4h14"/></svg>' }
  },
  
  init() {
    this.panel = document.getElementById('favoritesPanel');
    this.closeBtn = document.getElementById('favoritesClose');
    ...
    document.addEventListener('click', (e) => {
      if (this.panel && this.panel.classList.contains('active')) {
        if (!this.panel.contains(e.target) && !e.target.closest('.favorite-btn')) {
          this.hide();
        }
      }
    });
  ...
  loadFavorites() {
    try {
      const saved = localStorage.getItem('glp1-favorites');
  ...
  saveFavorites() {
    localStorage.setItem('glp1-favorites', JSON.stringify(this.favorites));
  },
  
  add(pageId) {
    if (!this.favorites.includes(pageId)) {
  ...
  remove(pageId) {
    this.favorites = this.favorites.filter(f => f !== pageId);
    this.saveFavorites();
    this.render();
  ...
  isFavorite(pageId) {
    return this.favorites.includes(pageId);
  },
  ...
  updateFavoriteButtons() {
    document.querySelectorAll('.favorite-btn').forEach(btn => {
      const pageId = btn.dataset.page;
      if (pageId) {
        btn.classList.toggle('active', this.isFavorite(pageId));
      }
    });
  },
    ...
    if (this.favorites.length === 0) {
      this.listEl.innerHTML = `
        <div class="favorites-empty">
          ...
          <span>Click the star icon on any page to add it here</span>
        </div>
      `;
      return;
    }
    
    this.listEl.innerHTML = this.favorites.map(pageId => {
      const info = this.pageInfo[pageId] || { title: pageId, category: 'Page', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>' };
      return `
        <div class="favorite-item" data-page="${pageId}">
          <div class="favorite-item-icon">${info.icon}</div>
          <div class="favorite-item-content">
            <div class="favorite-item-title">${info.title}</div>
            <div class="favorite-item-category">${info.category}</div>
          </div>
          <button class="favorite-item-remove" data-remove="${pageId}" aria-label="Remove from favorites">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="18" y1="6" x2="6" y2="18"/>
              <line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
    ...
    this.listEl.querySelectorAll('.favorite-item').forEach(item => {
      item.addEventListener('click', (e) => {
        if (e.target.closest('.favorite-item-remove')) {
          e.stopPropagation();
          const pageId = e.target.closest('.favorite-item-remove').dataset.remove;
          this.remove(pageId);
          this.updateFavoriteButtons();
        } else {
          const pageId = item.dataset.page;
...
// ============================================
function copyMacroText(macroId) {
  const macroBlock = document.querySelector(`[data-macro-id="${macroId}"]`);
  if (!macroBlock) return;
  
  const textContent = macroBlock.querySelector('.macro-text, .macro-content');
  if (!textContent) return;
  
  const text = textContent.innerText || textContent.textContent;
  
  navigator.clipboard.writeText(text).then(() => {
    ToastManager.show('Macro copied to clipboard', 'success');
    
    // Update button state
    const btn = macroBlock.querySelector('.macro-copy-btn');
    if (btn) {
      btn.classList.add('copied');
...
window.copyMacroText = copyMacroText;
